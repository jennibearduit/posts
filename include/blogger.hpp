...
class [[eosio::contract("blogger")]] blogger : public eosio::contract {
   public:
      ...
      [[eosio::action]] void addpost(eosio::name user, std::string post_content);
      [[eosio::action]] void deletepost(uint64_t id, eosio::name user); // User here is the post creator, needed for require_auth
      [[eosio::action]] void ratepost(uint64_t id, eosio::name user, int rating_score); // User here is the person rating the post, needed for require_auth and table
